...
{p 4 4 2}
Optionally, users can request inflation adjustment factors {hline 1} used to adjust nominal dollars to real dollars {hline 1} pegged to a given base year. For instance, {bf:get_cpiu, base_year(2018) clear} loads the CPI-U data series as variable {c 39}cpiu{c 39} and also creates another variable, {c 39}cpiu_2018_adj{c 39}. Multiplying nominal dollar amounts by {c 39}cpiu_2018_adj{c 39} will yield 2018 inflation-adjusted dollar amounts. 

{p 4 4 2}
The program automatically caches the retreived data series. Users can load cached data by specifying {bf:use_cache}; {bf:get_cpiu} will display the date when cached data was originally downloaded. To refresh the cached data with the latest available from the BLS, run {bf:get_cpiu} without {bf:use_cache} (you must be connected to the internet). 
...
{syntab:Optional}
	...
	{synopt:{opt yearvar:name}}if {bf:merge} is specified, the variable on which to merge the inflation data into data in memory; default is {opt yearvar(year)}.{p_end}
