...
{p 4 4 2}
CBPP{c 39}s standard profile.do defines global macros that serve as shortcuts to synched SharePoint and OneDrive folders and cloned GitHub repositories. It also defines a global macro named {c 39}censuskey{c 39} that contains the user{c 39}s Census Bureau API key. Users may specify their API key in the {it:censuskey()} option to  insert it into the profile.do. If this option is not specified, the {c 39}censuskey{c 39} global will contain placeholder text.
...
{synopthdr}
{synoptline}
	{synopt:{opt censuskey}}Census Bureau API key.{p_end}
