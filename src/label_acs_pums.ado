...
__label_acs_pums__ labels American Community Survey [public use microdata](https://www.census.gov/programs-surveys/acs/technical-documentation/pums.html) using information compiled from the ACS PUMS [data dictionaries](https://www2.census.gov/programs-surveys/acs/tech_docs/pums/data_dict/).

The program automatically caches the retrived data dictionary and labels. To avoid downloading and generating the files each time, specify the __use_cache__ option. If you do not specify __use_cache__, or if you have never run __label_acs_pums__ for the year and sample specified, you must be connected to the internet to use this program.
