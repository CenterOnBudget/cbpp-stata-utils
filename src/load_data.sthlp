...
{p 4 4 2}
{bf:load_data} loads CPS, ACS, SNAP QC, or Household Pulse Survey microdata from the CBPP datasets library into memory. This command is only useful for CBPP staff.
...
{p 4 4 2}
Users may specify a single year or multiple years to {bf:years()} as a {help numlist}. With {opt dataset(pulse)}, users can specify the weeks of data to retrieve in either {bf:weeks()} or {bf:years()}. If multiple years are specified, the datasets will be appended together before loading and retain variable and value labels from the maximum year in {bf:years()}. 
...
{p 4 4 2}
Note: When loading multiple years of ACS datasets including 2018 and later samples, {c 39}serialno{c 39} will be edited to facilitate appending ({c 39}serialno{c 39} is string in 2018 and later datasets, and numeric in prior years{c 39} datasets): "00" and "01" will be substituted for "HU" and "GQ", respectively, and the variable will be destringed.    {break}
