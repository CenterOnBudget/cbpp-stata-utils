...
{syntab:Required: At least one of the following options must be specified}
	...
	{synopt:{opt replace}}replace data in memory with the inflation series. Cannot be combined with 'merge'.{p_end}
...
----------
    ...
    Retreive CPI-U-RS and inflation-adjustment factors for 2018 dollars, replacing data in memory.
        {bf:. get_cpiu, rs base_year(2018) replace}
...
program define get_cpiu
	...
	* download if needed, then load -------------------------------------------
	...
	if !`download' {
		...
		// TODO add check if cached dataset is old enough that a new years' worth
		// of data is out
	}
