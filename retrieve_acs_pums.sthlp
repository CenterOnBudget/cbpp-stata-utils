...
{p 4 4 2}
{bf:retrieve_acs_pums} {hline 2} Retrieve American Community Survey PUMS files from the Census Bureauy FTP.



{title:Description}
{break}    6
{bf:retrieve_acs_pums} downloads American Community Survey  {browse "https://www.census.gov/programs-surveys/acs/technical-documentation/pums.html":public use microdata} files from the  {browse "https://www.census.gov/programs-surveys/acs/data/data-via-ftp.html":Census Bureau FTP} and creates {help dta} versions of the files.    {break}

{p 4 4 2}
Data are saved in acs_pums/[year]/[sample]_yr within the current working directory (the default) or in another "destination directory" the user specifies with the {bf:dest_dir} option. For instance, the command {bf:retrieve_acs_pums, state(vt) year(2018) sample(5) record_type(h) dest_dir(my_data)} would save files in my_data/acs_pums/2018/5_yr, creating directories as needed.
