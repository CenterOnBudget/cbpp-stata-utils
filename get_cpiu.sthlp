...
{p 4 4 2}
Optionally, users can request inflation adjustment factors--used to adjust nominal dollars to real dollars--pegged to a given base year. For instance, {bf:get_cpiu, base_year(2018) replace} loads the CPI-U data series as variable {it:cpiu} and also creates another variable, {it:cpiu_2018_adj}. Multiplying nominal dollar amounts by {it:cpiu_2018_adj} yields 2018 inflation-adjusted dollar amounts. 
...
{syntab:Required: At least one of the following options must be specified}
	...
	{synopt:{opt replace}}replace data in memory with the inflation series. Cannot be combined with {c 39}merge{c 39}.{p_end}
